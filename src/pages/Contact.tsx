...
/**
 * Genera un enlace de WhatsApp simulado
 * En producción se usaría el número real del centro
 */
const generateWhatsAppLink = (phone: string, centerName: string): string => {
    // Limpiar el número de teléfono
    const cleanPhone = phone.replace(/[^0-9]/g, '');
    const message = encodeURIComponent(
        `¡Hola! Me comunico desde SaludConecta VE. Quisiera información sobre citas médicas en ${centerName}.`
    );
...
const ContactPage: React.FC<ContactPageProps> = ({
    ...
    // Simular apertura de WhatsApp (en demo solo muestra alerta)
    const handleWhatsAppClick = (centerName: string, phone: string) => {
        const link = generateWhatsAppLink(phone, centerName);
        // En modo demo mostramos una alerta
        alert(
            `📱 Simulación de WhatsApp\n\n` +
            `Centro: ${centerName}\n` +
            `Teléfono: ${phone}\n\n` +
            `En un entorno real, se abriría:\n${link}`
        );
    };
            ...
            <main className="max-w-2xl mx-auto px-4 py-6">
                        ...
                        >
                            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                                ...
                                <div className="flex-shrink-0">
                                    <Button
                                        variant="primary"
                                        onClick={() => handleWhatsAppClick(center.nombre, center.telefono)}
                                        leftIcon={
                ...
                <div className="mt-8 p-4 bg-amber-50 rounded-xl border border-amber-200">
                    <p className="text-sm text-amber-800">
                        <strong>📋 Nota de demostración:</strong> Los botones de WhatsApp
                        están simulados para la presentación académica. En un entorno de
                        producción, abrirían directamente la aplicación de WhatsApp con
                        el mensaje pre-configurado.
                    </p>
                </div>
            </main>
        </div>
